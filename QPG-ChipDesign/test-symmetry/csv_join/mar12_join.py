...
if __name__ == "__main__":
  ...
  if k=='mar12':   lbls = {'mar12_type1_alt': 'Optimize 1 state w/ Square', 'mar12_type2_alt': 'Optimize both hyperfines w/ Square', 'mar12_type3_alt': 'Optimize all w/ regularization', 'mar12_type4_bigLR': 'Optimize all w/ FFT+hann'}
                           ...
                           'jun10_plots_1step_shro': '1 (1)',
                           'jun10_plots_1step_3pi_shro': '1 (2)',
                           'jun10_plots_1step_1.1': '1 (1)', 
                           'jun10_plots_1step_3.3': '1 (1)', 
                           'jun10_plots_1step_5pi_shro': '1 (3)',
                           # 'mar31_type_2': '2 (1)',
                           # 'mar31_type_2_2osc': '2 (2)',
                           # 'mar31_type_2_3osc': '2 (3)',
                           'jun10_plots_100step_1.1': '3 (1)', 
                           'jun10_plots_100step_3.3': '3 (1)', 
                           'jun10_plots_guess10_lr5_shro': '3 (1)', 
                           # 'mar31_type_3_test': '3 (3)', 
                           }
  elif k=='sym_tests': lbls = {
                           ...
                           # 'mar31_type_2_2osc': '2 (2)',
                           # 'mar31_type_2_3osc': '2 (3)',
                           'jun10_plots_100step_1.1': 'Only reg. (1)', 
                           'jun10_plots_100step_3.3': 'Only reg. (1)', 
                           'jun10_plots_guess10_lr5_shro': 'Only reg. (1)', 
                           'test-symmetry-1': 'Constrained + reg. (1)', 
                           # 'mar31_type_3_test': '3 (3)', 
                           }
  ...
  # 	print(grp[['T', 'data:CUSTOM_splitting', 'mar12_loss']])

  if True: df["data:CUSTOM_splitting"] = round(df["data:CUSTOM_splitting"]/(2 * np.pi), 2)

  if k in {'mar17', 'mar31'}:
    df = df.loc[df["data:T"] >= 0.2]

  df['Spin Splitting $\\Delta$ (MHz)'] = df['data:CUSTOM_splitting'].apply(lambda x: str(x))
  ...
  with pd.option_context('display.max_rows', None, 'display.max_columns', None):  # more options can be specified also
    print(df[['mar_sim_type', 'Initial Conditions', 'Number of Oscillations', 'mar12_loss']])#, 'data:T', 'Maximum Amplitude'

  col_order = [str(i) for i in sorted(df['data:CUSTOM_splitting'].unique())]
  ...
  if k=='mar31':
    ...
    # - df.loc[df['Optimization Type'] == 'mar31_type_1' and df['Initial Conditions'] == row['Initial Conditions'] and df['data:CUSTOM_splitting']==row['data:CUSTOM_splitting'] and df['data:T']==row['data:T'] ]

    df['Relative Loss'] = df.apply(lambda row: row['mar12_loss'] - df.loc[
                (df['Optimization Type'] == '1') & 
                (df['Number of Oscillations'] == '1') & 
                (df['data:CUSTOM_splitting']==row['data:CUSTOM_splitting']) & 
